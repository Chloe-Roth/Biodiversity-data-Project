...
# We assume that matrix_full_final is a data frame containing at least:
# - longitude
# - latitude
# - species
#
...
spatial_points <- SpatialPoints(
  coords = matrix_full_final[, c("longitude", "latitude")],
...
# --> We check that our points are good place in our map
...
# --> we extract the climate data in each location 
# -->(detection of the point and extraction of climate data in this point)
...
matrix_full_eco <- data.frame(matrix_full_final, eco_values)
...
# --> control to ckeck that all is logical